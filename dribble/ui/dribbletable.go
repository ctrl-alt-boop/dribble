...
func (t *DribbleTable) View() string {
	...
	for i, column := range t.Columns {
		headerCells[i] = headerStyle.Width(column.Width).MaxWidth(column.Width).Render(column.Value)
	}
	...
	for i, row := range t.Rows {
		...
		for j, cell := range row {
			...
			cells[j] = style.Width(cellSize).Render(value)
		}
...
func (t *DribbleTable) getColumnsAsTableColumns(dataTable data.Table) Columns {
	...
	for _, column := range columns {
		tableColumns = append(tableColumns, Column{Value: column.First, Width: column.Second + 2})
	}
	return tableColumns
}
